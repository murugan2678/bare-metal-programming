...
{
  for(int i = 0; i < ms * 4000; i++) /* 1000 means 1 milliseconds */
  {
...
void seven_segment_led()
{
  ...
  for (int m = 0; m < 10; m++)  /* 7segment 0 to 7 bit. totally 8-bit data */
  {
    *GPIOA_ODR = arr[m];  /* 7segment display value load. Display ON */
    delay_ms(200);        /* delay 200 milliseconds stop */
  }
}
