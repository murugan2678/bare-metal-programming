 ...
 *  Author      : Murugan M                                                   *
 *  Date        : 14-02-2025                                                  *
 *  File        : timer1_HSI_internal_clock_1seconds                          *
 *  Description : Led blink for PORTA 2nd pin.                                *
...
void gpioa_moder_config(void)
{
	...
	GPIOA->MODER |= (1 << 4);  // MODER 2 select. 01: General purpose output
}

void timer1_congfig(void)
{
...
void led_blinking(void)
{
	...
	/* 1seconds led on. 1seconds value : 16000 */
	timer1_delay(16000);
