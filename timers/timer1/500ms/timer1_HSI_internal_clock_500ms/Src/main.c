 ...
 *  Author      : Murugan M                                                   *
 *  Date        : 14-02-2025                                                  *
 *  File        : timer1_HSI_internal_clock_500ms                             *
 *  Description : Led blink for PORTA 1st pin.                                *
 *                using for two pheripheral bus AHB1ENR and APB2ENR           *
 *	           	  i give 500ms milliseconds              			          *
...
void gpioa_moder_config(void)
{
	...
	GPIOA->MODER |= (1 << 2); // MODER 1 select. 01: General purpose output
}
...
void led_blinking(void)
{
	...
	/* PORT A 2nd pin set 1. led on*/
	GPIOA->ODR |= (1 << 1);

	/* 500ms led on. 500ms milliseconds value : 8000 */
	timer1_delay(8000);

	/* PORT A 2nd pin set 0. led off */
	GPIOA->ODR &= ~(1 << 1);
